...
def choosewim():
  wimdir = filedialog.askdirectory()
  wimbrowse2.configure(text=wimdir)  
def choosegho():
  ghostdir = filedialog.askopenfilename()
  ghobrowse2.configure(text=ghostdir) 
def run():
  title.configure(text="Done!")
  ...
  dism = "dism /Capture-Image" + " /ImageFile:" + "\"" +  wimdir + "" + "install.wim" + "\"" + " /CaptureDir:" + windir + ":"+ " /Name:" + "\"" + ver + "\""
...
temp = customtkinter.CTkLabel(app, text="See the progress when running in the command window!")
temp.pack(padx=10, pady=10)
wimbrowse = customtkinter.CTkButton(app, text="Start", command=run)
wimbrowse.pack(padx=10, pady=10)
title = customtkinter.CTkLabel(app, text="Made with ♥ by Random Tech Channel")
